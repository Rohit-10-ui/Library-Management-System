...
const actualRole = storedUser?.role || role;
const actualUserName = storedUser?.username || userName;
const actualUserId = storedUser?.userId || userId;

const isStaff = actualRole === 'admin' || actualRole === 'librarian';
...
const [returnStatus, setReturnStatus] = useState('RETURNED');
  const members = [
    {id:1,name:'kit',    status:'active',membershipType:'premium'},
    {id:2,name:'charlie',  status:'active',membershipType:'standard'},
    {id:3,name:'pip',status:'active',membershipType:'basic'},
    {id:4,name:'prash',    status:'active',membershipType:'premium'},
    {id:5,name:'bright',  status:'active',membershipType:'standard'},
    {id:6,name:'dhruv1234',status:'active',membershipType:'basic'},
    {id:4,name:'ravi',    status:'active',membershipType:'premium'},
...
const fetchIssues = useCallback(async () => {
  ...
  const fd = new FormData(e.target);
  const selectedUsername = fd.get('username');   // ← get dropdown value

  try {
    const res = await api.issueBook({
      bookId: issueBook.id,
      username: selectedUsername                // ← send selected username
    });
    ...
    setIssues(prev => [
      ...prev,
      {
        ...
        username: selectedUsername,             // ← ensure it appears in response
...
const submitReturn = async (e) => {
      ...
      <div className={`book-card ${viewMode} card-enter`} style={{animationDelay:`${idx*0.045}s`}}>
        ...
        <div className="book-info">
          ...
          <div className="b-actions">
            {actualRole !== 'user' && (
  a>0
    ? <button className="btn-issue" onClick={()=>setIssueBook(book)}>
        <Ic.BookOpen s={14}/>Issue Book
      </button>
    : <button className="btn-un" disabled>
        <Ic.Clock s={14}/>Unavailable
      </button>
)}
          </div>
        </div>
  ...
  const IssRow=({rec,showMem=true})=>(
    <tr className={`rec-row rs-${rec.status}`}>
      <td><div className="ct">{rec.bookTitle}</div>{showMem&&<div className="cs">{rec.username}</div>}</td>
  ...
  ...(actualRole !== 'user'
    ...
    : []
        ...
        <div className="nav-right">
          <div className={`role-badge role-${actualRole}`}>{actualRole}</div>

<div className="user-pill">
  <div className="user-av"><Ic.User s={16}/></div>
  <span>{actualUserName}</span>
      ...
      {/* ═══ MODALS ═══ */}
{returnRec && (
  <div className="overlay" onClick={() => setReturnRec(null)}>
    <div className="modal iss-modal" onClick={e => e.stopPropagation()}>
      <div className="modal-head">
        <h2>Return Book</h2>
        <button className="mx" onClick={() => setReturnRec(null)}>
          <Ic.Close s={17} />
        </button>
      </div>

      <div style={{ padding: '1.5rem 2rem' }}>
        <h3>{returnRec.bookTitle}</h3>
        <p>User: {returnRec.username}</p>
      </div>

      <form onSubmit={submitReturn} className="bk-form" style={{ padding: '0 2rem 2rem' }}>
        <div className="fg">
        ...
        <div className="form-actions">
          <button type="button" className="btn-cancel" onClick={() => setReturnRec(null)}>
            Cancel
          </button>
          <button type="submit" className="btn-submit" disabled={actionLoad}>
            {actionLoad ? <Ic.Loader s={14} /> : 'Confirm'}
          </button>
        </div>
      </form>
    </div>
            ...
            <div className="det-wrap">
              ...
              <div className="det-info">
                ...
                <div className="det-acts">
                  {actualRole !== 'user' && (detailBook.availability??0)>0 &&
  <button className="btn-add" onClick={()=>{setDetailBook(null);setIssueBook(detailBook);}}>
    <Ic.BookOpen s={16}/>Issue Book
  </button>}
...
{issueBook && (
  <div className="overlay" onClick={() => setIssueBook(null)}>
    <div className="modal iss-modal" onClick={e => e.stopPropagation()}>
      <div className="modal-head">
        <h2>Issue Book</h2>
        <button className="mx" onClick={() => setIssueBook(null)}>
          <Ic.Close s={17} />
        </button>
      </div>
      ...
      <div className="iss-prev">
        ...
        <div>
          <h3>{issueBook.title}</h3>
          <p>{issueBook.author}</p>
          <span className="av-tag">
            {issueBook.availability} available
          </span>
        </div>
      </div>

...
const api = {
  getMemberships: () =>
    authFetch(`${BASE_URL}/memberships`).then(r => r.json()),

  getMemberHistory: (userName) =>
    authFetch(`${BASE_URL}/members/${userName}/history`).then(r => r.json()),

  assignMembership: (userName, membershipId) =>
    authFetch(`${BASE_URL}/memberships/assign`, {
      method: "POST",
      body: JSON.stringify({ username:userName, membershipId })
    }).then(r => r.text()),

  getIssues: () =>
    authFetch(`${BASE_URL}/issues`).then(r => r.json()),

  getMembersByPlan: (planId) =>
  authFetch(`${BASE_URL}/memberships/${planId}/members`).then(r=>r.json()),

createMembership: (data) =>
  authFetch(`${BASE_URL}/memberships`, {
    method:"POST",
    body:JSON.stringify(data)
  }).then(r=>r.json()),

updateMembership: (id,data) =>
  authFetch(`${BASE_URL}/memberships/${id}`,{
    method:"PUT",
    body:JSON.stringify(data)
  }).then(r=>r.json()),
...
deleteMembership: (id) =>
  ...
  getMembersByPlan: (id) =>
  authFetch(`${BASE_URL}/memberships/${id}/members`)
    .then(r => r.json()),
};
...
useEffect(() => {
  if (role === "USER" && tab === "members") {
    setTab("profile");
  }
  if (role === "USER" && tab === "plans") {
    setTab("profile");
  }
}, [role, tab]);
...
const upgrade = async (planId) => {
  ...
  setActionLoad(true);
  try {
    await api.assignMembership(userName, planId);
...
const fetchHistory = useCallback(async () => {
  setHistLoad(true);
  try {
    const d = await api.getMemberHistory(userName);

    const mapped = (Array.isArray(d) ? d : []).map(i => {
      const today = new Date();
      const due = new Date(i.dueDate);
      const returnDate = i.returnDate ? new Date(i.returnDate) : null;

      let daysOverdue = 0;
      if (i.status === 'RETURNED' && returnDate > due) {
        daysOverdue = Math.ceil((returnDate - due) / 86400000);
      }
      if (i.status === 'ISSUED' && today > due) {
        daysOverdue = Math.ceil((today - due) / 86400000);
      }
  ...
  .filter(i => i.username === userName );
      ...
      setActiveIss([
        {id:3,bookId:4,bookTitle:'Clean Code',issueDate:'2024-01-10',dueDate:'2024-02-10',returnDate:null,status:'ISSUED',daysOverdue:0,lateFee:0},
      ]);
    }
  },[userName]);
  ...
  const fetchPlanMembers = async(plan)=>{
    ...
    try{const d=await api.getMembersByPlan(plan.id);setPlanMembers(d?.content || []);}
  ...
  const submitPlan = async e=>{
    ...
    const data={
      name:fd.get('username'),borrowLimit:parseInt(fd.get('borrowLimit')),
  ...
  ...(isStaff ? [
    ...
    {k:'members', label:'All Members', icon:<Ic.Users s={15}/>},
  ] : [])
].map(t => (
                  ...
                  {plans.map((plan,i)=>{
                    ...
                    return(
                      <div key={plan.id} className={`plan-card card-enter ${isMyPlan&&!isStaff?'my-plan':''}`} style={{'--pc':col,animationDelay:`${i*0.07}s`}}>
                        {role === "USER" && isMyPlan && <div className="cur-badge"><Ic.Star s={12} f/> Current Plan</div>}
                        ...
                        {role === "USER" && !isMyPlan && <button onClick={()=>upgrade(plan.id)} className="btn-upgrade">Upgrade to this plan</button>}
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="profile-layout">
              ...
              <div className="profile-right">
                <div className="stat-cards">
                  {[
                    {label:'Books Issued',val:myMembership.borrowedBooks,icon:<Ic.BookOpen s={22} c="#FF9B7A"/>,col:'#FF9B7A'},
                ...
                <div className="active-iss-card card-enter">
                  <h3><Ic.BookOpen s={18} c="#FF9B7A"/>Currently Issued</h3>
                  {activeIss.length===0
                    ?<div className="no-active"><Ic.Books s={40} c="#FFD4B8"/><p>No books currently Issued</p></div>
        ...
        {tab==='history'&&(
          <div className="mp-content">
            ...
            <div className="hist-summary card-enter">
              {[
                {l:'Total Issued', v:history.length,                                      c:'#6B9BD1'},
                {l:'Returned',       v:history.filter(h=>h.status==='RETURNED').length,     c:'#27AE60'},
                {l:'Active',         v:history.filter(h=>h.status==='ISSUED').length,     c:'#3498DB'},
                          ...
                          :planMembers.map(m=>(
                            <div key={m.id} className="pms-mem-row">
                              ...
                              <div className="pmm-info">
                                <strong>{m.firstName||m.username}</strong>
                                <span>{m.email}</span>
                              </div>

...
const fetchDashboard = async () => {
  try {
    const data = await api("/librarian/dashboard");
    setDashboard(data);
  } catch {
...
const fetchPlans = async () => {
  try {
    ...
    toast("Failed to fetch plans", "error");
  }
};
...
const fetchBooks = async () => {
  try {
    const data = await api("/books?page=0&size=50");
...
const fetchIssues = async () => {
  try {
    ...
    toast("Failed to fetch issues", "error");
  }
};
...
const getUserHistory = async (username) => {
  return await api(`/members/${username}/history`);
};
    ...
    <div className="ad-root">
      ...
      <main className="ad-main">
        ...
        <div className="ad-content">
          ...
          {activeNav==="books" && <BooksSection books={books}onAdd={() => navigate("/books")} onEdit={()=>toast("Edit book")} onDelete={deleteBook}/>}
